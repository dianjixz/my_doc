...
sha224_hash=$(echo -n "$input_string" | sha256sum | cut -d' ' -f1 | tr 'a-z' 'A-Z')

i=0
while [ $i -lt 4 ]; do
    start=$((i * 12))
    end=$((start + 12))
    oui_bytes=$(echo "$sha224_hash" | cut -c $((start + 1))-$((end)))
    tmp_str1="$(echo $oui_bytes | sed -r 's/(..)(..)(..)(..)(..)(..)/\1:\2:\3:\4:\5:\6/')"
    mac=$(echo "$tmp_str1" | sed "s/^...../10:E7/")
    eval "mac$i=$mac"
    i=$((i + 1))
done
